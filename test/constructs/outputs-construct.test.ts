...
describe('Outputs Construct', () => {
  ...
  test('creates CloudFormation outputs for true 1-tier architecture', () => {
    ...
    // No database outputs in true 1-tier architecture
  });
  ...
  test('exports VPC information', () => {
    ...
    expect(outputs[vpcIdOutput!].Export.Name).toBe('test-HighlyAvailable1Tier-VpcId');
    expect(outputs[vpcCidrOutput!].Export.Name).toBe('test-HighlyAvailable1Tier-VpcCidr');
  });
  ...
  test('exports load balancer DNS', () => {
    ...
    expect(outputs[loadBalancerDnsOutput!].Export.Name).toBe('prod-HighlyAvailable1Tier-LoadBalancerDNS');
  });
});
